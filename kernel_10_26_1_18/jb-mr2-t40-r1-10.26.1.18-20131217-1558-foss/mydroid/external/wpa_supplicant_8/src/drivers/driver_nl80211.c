...
static int wpa_supplicant_project_p2p_cap_mcc() {
	char project_id[PROPERTY_VALUE_MAX];
	char property_p2p_mcc[PROPERTY_VALUE_MAX];
	property_get("ro.product.model", project_id, "0");
	property_get("persist.p2p.mcc", property_p2p_mcc, "0");

	if (os_strncmp(property_p2p_mcc, "1", 1) == 0) {
		wpa_printf(MSG_INFO, "Enable MCC from property");
		return 1;
	}

	if (os_strncmp(project_id, "K00C", 4) == 0) { //TF701T
		wpa_printf(MSG_INFO, "Disable MCC for K00C");
		return 0;
	}
	return 0;
}
...
static void wpa_supplicant_init_device_cap(void *priv)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;

	wpa_printf(MSG_INFO, "ifname = %s", bss->ifname);

	if(os_strncmp(bss->ifname, "wlan0", 5) == 0) {
		if (wpa_supplicant_project_p2p_cap_mcc()) {
			wpa_printf(MSG_INFO, "wlan0: Use Multi channel concurrency");
			drv->capa.flags |= WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT;
		}
	}
	if(os_strncmp(bss->ifname, "p2p0", 4) == 0) {
		if (wpa_supplicant_project_p2p_cap_mcc()) {
			wpa_printf(MSG_INFO, "p2p0: Use Multi channel concurrency");
			drv->capa.flags |= WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT;
		}
	}
}
...
static int nl80211_set_param(void *priv, const char *param)
{
	...
	wpa_supplicant_init_device_cap(priv);
	if (param == NULL)
		return 0;
