				    ...
				    unsigned int pref_freq)
{
	...
	wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "p2p_prepare_channel_pref");
...
static void p2p_prepare_channel_best(struct p2p_data *p2p)
{
	...
	if (!p2p->cfg->cfg_op_channel && p2p->best_freq_overall > 0 &&
	    ...
	    p2p_freq_to_channel(p2p->cfg->country, p2p->best_freq_overall,
				&op_class, &op_channel) == 0) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Select best "
		...
		p2p->op_channel = op_channel;
		   ...
		   p2p_freq_to_channel(p2p->cfg->country, p2p->best_freq_5,
				       &op_class, &op_channel) == 0) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Select best 5 GHz "
		...
		p2p->op_channel = op_channel;
		   ...
		   p2p_freq_to_channel(p2p->cfg->country, p2p->best_freq_24,
				       &op_class, &op_channel) == 0) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Select best 2.4 "
...
void p2p_set_best_channels(struct p2p_data *p2p, int freq_24, int freq_5,
			   int freq_overall)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Best channel: 2.4 GHz: %d,"
