			    ...
			    struct p2p_channels *res)
{
	...
	wpa_printf(MSG_INFO, "P2P: ======== ");

	for (i = 0; i < a->reg_classes; i++) {
		const struct p2p_reg_class *aa_reg = &a->reg_class[i];
		wpa_hexdump(MSG_INFO, "P2P: channels of A", aa_reg->channel, aa_reg->channels);
	}

	for (j = 0; j < b->reg_classes; j++) {
		const struct p2p_reg_class *bb_reg = &b->reg_class[j];
		wpa_hexdump(MSG_INFO, "P2P: channels of B", bb_reg->channel, bb_reg->channels);
	}
