				...
				const u8 *data, size_t len, int rx_freq)
{
	...
	if (msg.invitation_flags)
		 ...
		 * missing.
		 */
		wpa_printf(MSG_INFO, "P2P: Mandatory Invitation Flags "
	...
	if (op_freq) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Invitation "
		...
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Own default "
		...
		if (msg.operating_channel) {
			...
			wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Peer "
	       ...
	       int persistent_group, unsigned int pref_freq)
{
	...
	wpa_hexdump_ascii(MSG_INFO, "P2P: Invitation for SSID",
			  ssid, ssid_len);
