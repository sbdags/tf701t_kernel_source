...
static int p2p_go_det(u8 own_intent, u8 peer_value)
{
	...
	wpa_printf(MSG_INFO, "own_intent = %d, peer_intent = %d", own_intent, peer_intent);
			  ...
			  struct p2p_channels *intersection)
{
	...
	unsigned int i;
        ...
        wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "p2p_reselect_channel");                                                                                
        wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Selected operating "
	...
	if (p2p->own_freq_preference > 0 &&
		...
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Pick own channel "
	...
	if (p2p->best_freq_overall > 0 &&
	    p2p_freq_to_channel(p2p->cfg->country, p2p->best_freq_overall,
				&op_reg_class, &op_channel) == 0 &&
	    p2p_channels_includes(intersection, op_reg_class, op_channel)) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Pick best overall "
	...
	if (freq >= 2400 && freq < 2500 && p2p->best_freq_5 > 0 &&
	    ...
	    p2p_freq_to_channel(p2p->cfg->country, p2p->best_freq_5,
				&op_reg_class, &op_channel) == 0 &&
	    p2p_channels_includes(intersection, op_reg_class, op_channel)) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Pick best 5 GHz "
			"channel (reg_class %u channel %u) from intersection",
			op_reg_class, op_channel);
		p2p->op_reg_class = op_reg_class;
		p2p->op_channel = op_channel;
	...
	if (freq >= 4900 && freq < 6000 && p2p->best_freq_24 > 0 &&
	    ...
	    p2p_freq_to_channel(p2p->cfg->country, p2p->best_freq_24,
				&op_reg_class, &op_channel) == 0 &&
	    p2p_channels_includes(intersection, op_reg_class, op_channel)) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Pick best 2.4 GHz "
			"channel (reg_class %u channel %u) from intersection",
			op_reg_class, op_channel);
		p2p->op_reg_class = op_reg_class;
		p2p->op_channel = op_channel;
	...
	for (i = 0; p2p->cfg->pref_chan && i < p2p->cfg->num_pref_chan; i++) {
		if (p2p_channels_includes(intersection,
			...
			wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Pick "
	...
	wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Pick another channel "
				 ...
				 u8 *status)
{
	...
	for (i = 0; i < intersection.reg_classes; i++) {
		...
		wpa_printf(MSG_INFO, "P2P: reg_class %u", c->reg_class);
		wpa_hexdump(MSG_INFO, "P2P: channels",
	...
	if (!p2p_channels_includes(&intersection, p2p->op_reg_class,
		...
		if (dev->flags & P2P_DEV_FORCE_FREQ) {
			*status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
			wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Peer does "
		...
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Try to optimize "
			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		if (!p2p->go_neg_peer) {
			wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Starting "
			     ...
			     const u8 *data, size_t len)
{
	...
	if (p2p->pending_action_state == P2P_PENDING_GO_NEG_RESPONSE) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Stopped waiting "
	 ...
	 */
	wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: 20 ms wait on current "
