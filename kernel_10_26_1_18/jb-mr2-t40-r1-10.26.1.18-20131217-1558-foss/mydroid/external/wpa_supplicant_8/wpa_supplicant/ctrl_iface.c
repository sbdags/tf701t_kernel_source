...
static int wpa_supplicant_global_iface_interfaces(struct wpa_global *global,
						  char *buf, int len);
static void wpa_supplicant_scan_req(struct wpa_supplicant *wpa_s)
{
...
static void update_hw_feature_for_p2p_channels(struct wpa_supplicant *wpa_s)
{
	wpa_supplicant_scan_req(wpa_s);
	wpa_s->hw.modes = wpa_drv_get_hw_feature_data(wpa_s,
							&wpa_s->hw.num_modes,
							&wpa_s->hw.flags);
	wpa_printf(MSG_INFO,"update channel list");
			    ...
			    char *buf, size_t buflen)
{
	...
	update_hw_feature_for_p2p_channels(wpa_s);
	new_pin = wpas_p2p_connect(wpa_s, addr, pin, wps_method,
				   persistent_group, automatic, join,
				   auth, wpa_s->global->wifi_display ? 1 : go_intent, freq, persistent_id, pd,
				   ht40);
	if (new_pin == -2) {
