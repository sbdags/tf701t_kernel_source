...
static int wpas_channel_to_band(int channel)
{
	if (36 <= channel && 48 >= channel)
		return 1;

	if (149 <= channel && 161 <= channel)
		return 4;
                                   ...
                                   const u8 *peer)
{
	...
	if (status != P2P_SC_SUCCESS) {
		...
		if (status == P2P_SC_FAIL_NO_COMMON_CHANNELS) {
			wpa_printf(MSG_ERROR, "P2P: No common channel");
			if (wpas_channel_to_band(wpas_freq_to_channel(wpa_s->best_overall_freq)) == 1)
				wpa_s->p2p_no_common_channel_band1 = 1;
			else if (wpas_channel_to_band(wpas_freq_to_channel(wpa_s->best_overall_freq)) == 4)
				wpa_s->p2p_no_common_channel_band4 = 1;
		}
		return;
	}
...
static int is_oper_channel(struct wpa_supplicant *wpa_s, int freq)
{
	...
	for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
		...
		if (shared_freq) {
                        wpa_printf(MSG_DEBUG,"freq = %d, op channel = %d",freq, shared_freq);
			if (channel_to_freq(shared_freq) == freq)
				return 1;
		}
	}
	return 0;
...
static int is_channel_avaliable(struct wpa_supplicant *wpa_s, int channel)
{
	size_t i;
	int freq;
	struct wpa_bss *bss;
	...
	wpa_printf(MSG_DEBUG, "fetch scan results for %d", freq);

	if (is_oper_channel(wpa_s, freq)) {
		wpa_printf(MSG_INFO, "1. freq %d is avaliable", freq);
		return 1;
	}

	dl_list_for_each(bss, &wpa_s->bss_id, struct wpa_bss, list_id) {
		wpa_printf(MSG_DEBUG, "2. freq %d, SSID : %s", bss->freq, wpa_ssid_txt(bss->ssid, bss->ssid_len));
		if (bss->freq == freq) {
			wpa_printf(MSG_INFO, "2. freq %d is avaliable", freq);
			return 1;
		}
	}
	return 0;
...
static int has_channel(struct wpa_supplicant *wpa_s, struct wpa_global *global,
		       struct hostapd_hw_modes *mode, u8 chan, int *flags, u8 init)
{
	int i;
	unsigned int freq;
	struct wpa_scan_results *scan_res;
	...
	for (i = 0; i < mode->num_channels; i++) {
		if (mode->channels[i].chan == chan) {
			wpa_printf(MSG_DEBUG, "[CHANNEL] channel = %d, flag = %X\n",mode->channels[i].chan ,mode->channels[i].flag);
			if (flags)
			...
			return (!(mode->channels[i].flag &
				 (HOSTAPD_CHAN_DISABLED |
				  ...
				  HOSTAPD_CHAN_RADAR)) || (!init && is_channel_avaliable(wpa_s, mode->channels[i].chan)));
		}
	}
...
static int wpas_p2p_verify_channel(struct wpa_supplicant *wpa_s,
				   ...
				   u8 channel, u8 bw, u8 init)
{
	int flag;

	if (!has_channel(wpa_s, wpa_s->global, mode, channel, &flag, init))
		return -1;
	if (bw == BW40MINUS &&
	    (!(flag & HOSTAPD_CHAN_HT40MINUS) ||
	     !has_channel(wpa_s, wpa_s->global, mode, channel - 4, NULL, init)))
		return 0;
	if (bw == BW40PLUS &&
...
static int wpas_p2p_setup_channels(struct wpa_supplicant *wpa_s,
				   struct p2p_channels *chan, int init)
{
	struct hostapd_hw_modes *mode;
	int cla, op;
	...
	for (op = 0; op_class[op].op_class; op++) {
		...
		for (ch = o->min_chan; ch <= o->max_chan; ch += o->inc) {
			if (wpas_p2p_verify_channel(wpa_s, mode, ch, o->bw, init) < 1)
				continue;
			if (reg == NULL) {
	...
	for (op = 0; op_class[op].op_class; op++) {
		...
		for (ch = o->min_chan; ch <= o->max_chan; ch += o->inc) {
			if (o->mode != HOSTAPD_MODE_IEEE80211A ||
				...
				continue;
			ret = wpas_p2p_verify_channel(wpa_s, mode, ch, o->bw, 1);
			if (ret < 0)
				continue;
			else if (ret > 0)
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	if (wpas_p2p_setup_channels(wpa_s, &p2p.channels, 1)) {
				   ...
				   const struct p2p_channels *channels)
{
	...
	if (freq) {
		if (!freq_included(channels, freq)) {
			wpa_printf(MSG_INFO, "P2P: Forced GO freq %d MHz not "
				   "accepted", freq);
			return -1;
		}
		wpa_printf(MSG_INFO, "P2P: Set GO freq based on forced "
		...
		wpa_printf(MSG_INFO, "P2P: Set GO freq based on configured "
		...
		wpa_printf(MSG_INFO, "P2P: Set GO freq based on best overall "
		...
		wpa_printf(MSG_INFO, "P2P: Set GO freq based on best 2.4 GHz "
			   "channel %d MHz", params->freq);
	} else if (wpa_s->conf->p2p_oper_channel == 0 &&
		   wpa_s->best_5_freq > 0 &&
		   p2p_supported_freq(wpa_s->global->p2p,
		...
		wpa_printf(MSG_INFO, "P2P: Set GO freq based on best 5 GHz "
			   "channel %d MHz", params->freq);
	} else {
		int chan;
		...
		if (chan == 11) {
			wpa_printf(MSG_INFO, "P2P: No 2.4 GHz channel "
				   "allowed");
			return -1;
		}
		wpa_printf(MSG_INFO, "P2P: Set GO freq %d MHz (no preference "
			   "known)", params->freq);
	}
	...
	if (wpa_s->current_ssid && wpa_drv_get_bssid(wpa_s, bssid) == 0 &&
		...
		if (!p2p_supported_freq(wpa_s->global->p2p, wpa_s->assoc_freq)
			...
			if (wpa_s->drv_flags &
			    WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT) {
				wpa_printf(MSG_INFO, "P2P: Cannot force GO on "
					   "the channel we are already using "
			...
			wpa_printf(MSG_INFO, "P2P: Force GO on the channel we "
	...
	if (res > 0 && !freq &&
		...
		if (wpa_s->drv_flags &
		    WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT) {
			wpa_printf(MSG_INFO, "P2P: Cannot force GO on the "
		...
		wpa_printf(MSG_INFO, "P2P: Force GO on the channel we are "
		...
		if (!freq_included(channels, params->freq)) {
		   ...
		   !(wpa_s->drv_flags &
		     WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT)) {
		wpa_printf(MSG_INFO, "P2P: Cannot start P2P group on %u MHz "
...
static int driver_has_channel(struct wpa_supplicant *wpa_s, struct hostapd_hw_modes *mode, u8 chan)
{
	int i;
	unsigned int freq;
	struct wpa_scan_results *scan_res;
...
int wpas_select_best_freq(struct wpa_supplicant *wpa_s)
{
	int best_channel = 0, best_channel_val = 999, freq_container[166]= {0};
	unsigned int i = 0;
	struct wpa_bss *bss;
        struct hostapd_hw_modes *mode;
        int cla = 0, op;
	...
	if (wpa_s->hw.modes != NULL) {
		for (op = 0; op_class[op].op_class; op++) {
			...
			if (mode == NULL)
				continue;
			for (ch = o->min_chan; ch <= o->max_chan; ch += o->inc) {
				if (driver_has_channel(wpa_s, mode, ch)) {
					wpa_printf(MSG_DEBUG, "channel %d is supported by driver default", ch);
					if (ch < 14)
						freq_container[ch] = 10;
					if ((ch >= 36 && ch <= 48) || (ch >= 149 && ch <=161))
						freq_container[ch] = 1;
				}
	                }
	        }
	...
	dl_list_for_each(bss, &wpa_s->bss_id, struct wpa_bss, list_id) {
		wpa_printf(MSG_DEBUG, "3. freq %d, SSID : %s", bss->freq, wpa_ssid_txt(bss->ssid, bss->ssid_len));
		int channel = wpas_freq_to_channel(bss->freq);
		if (channel > 14)
			freq_container[channel] += 2;
		else
			freq_container[channel] += 5;
        }

	for (i = 1; i < 166; i++ ) {
		if (freq_container[i] == 0 || (i > 11 && i < 15)) {
			continue;
		}

		wpa_printf(MSG_DEBUG, "channel : %d  ct = %d", i, freq_container[i]);
		if (best_channel_val >= freq_container[i]) {
			if (wpa_s->p2p_no_common_channel_band1 && (wpas_channel_to_band(i) == 1)) {
			} else if (wpa_s->p2p_no_common_channel_band4 && (wpas_channel_to_band(i) == 4)) {
			} else {
				best_channel_val = freq_container[i];
				best_channel = i;
			}
		}
	}

	return channel_to_freq(best_channel);
}
...
void wpas_p2p_update_channel_list(struct wpa_supplicant *wpa_s)
{
	...
	if (wpas_p2p_setup_channels(wpa_s, &chan, 0)) {
	...
	oper_freq = wpa_drv_shared_freq(wpa_s);
	if (oper_freq <= 0)
		wpa_s->best_overall_freq = wpas_select_best_freq(wpa_s);
	else
		wpa_s->best_overall_freq = oper_freq;
	wpas_p2p_update_best_channels(wpa_s, wpa_s->best_24_freq, wpa_s->best_5_freq, wpa_s->best_overall_freq);
