...
public class DumpRenderTreeActivity extends Activity {
    ...
    private void setStateFromIntent() {
        ...
        mForceSWPixelTests = intent.getBooleanExtra("forceSWPixelTests", false);
        mForceSingleSurfaceRendering = intent.getBooleanExtra("forceSingleSurfaceRendering", false);
    }

    private void resetWebView() {
        DumpRenderTreeWebView oldView = mCurrentView;

        DumpRenderTreeWebView view = new DumpRenderTreeWebView(this, mResultHandler.obtainMessage(MSG_TEST_FINISHED), mForceSWPixelTests, mForceSingleSurfaceRendering, mStateListener);
    ...
    protected void onDestroy() {
        ...
        super.onDestroy();
        if (isFinishing()) {
            // SIGHUP handler will trigger dump of coverage stats
            // and delay will give time for dump to complete before
            // exiting
            Process.sendSignal(Process.myPid(), 1); // HUP
            try {
