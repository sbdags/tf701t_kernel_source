...
/** Implementation of JavaScript event sender object.
 * A class that acts as a JS interface for webview to mock various touch events,
 * mouse actions and key presses.
 *
 * This will be accessible in JavaScript environment as window.eventSender.
 */
public class EventSender {
    private static final String LOG_TAG = "EventSender";
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_KEY_DOWN:
                    ...
                    String str = bundle.getString("character");
                    ...
                    executeKeyEvent(KeyEvent.ACTION_DOWN,
                            stringToKeyCode(str));
                    executeKeyEvent(KeyEvent.ACTION_UP,
                            stringToKeyCode(str));
                    break;
    ...
    public EventSender(WebView webView) {
        mWebView = webView;
        mTouchPoints = null;
        mTouchMetaState = 0;
        mMousePoint = null;
     ...
     */
    private static int stringToKeyCode(String str) {
    ...
    /** Instance that forwards JS window.eventSender calls to Java.
     *
     * The methods here just call corresponding methods on EventSender
     * that contains the logic of how to execute the methods.
     */
    private Object mJSObject = new Object() {

        @JavascriptInterface
        public void enableDOMUIEventLogging(int domNode) {
            EventSender.this.enableDOMUIEventLogging(domNode);
        }

        @JavascriptInterface
        public void fireKeyboardEventsToElement(int domNode) {
            EventSender.this.fireKeyboardEventsToElement(domNode);
        }

        @JavascriptInterface
        public void keyDown(String character, String[] withModifiers) {
            EventSender.this.keyDown(character, withModifiers);
        }

        @JavascriptInterface
        public void keyDown(String character) {
            EventSender.this.keyDown(character, null);
        }

        @JavascriptInterface
        public void leapForward(int milliseconds) {
            EventSender.this.leapForward(milliseconds);
        }

        @JavascriptInterface
        public void mouseClick() {
            EventSender.this.mouseClick();
        }

        @JavascriptInterface
        public void mouseDown() {
            EventSender.this.mouseDown();
        }

        @JavascriptInterface
        public void mouseMoveTo(int x, int y) {
            EventSender.this.mouseMoveTo(x, y);
        }

        @JavascriptInterface
        public void mouseUp() {
            EventSender.this.mouseUp();
        }

        @JavascriptInterface
        public void touchStart() {
            EventSender.this.touchStart();
        }

        @JavascriptInterface
        public void addTouchPoint(int x, int y) {
            EventSender.this.addTouchPoint(x, y);
        }

        @JavascriptInterface
        public void updateTouchPoint(int id, int x, int y) {
            EventSender.this.updateTouchPoint(id, x, y);
        }

        @JavascriptInterface
        public void setTouchModifier(String modifier, boolean enabled) {
            EventSender.this.setTouchModifier(modifier, enabled);
        }

        @JavascriptInterface
        public void touchMove() {
            EventSender.this.touchMove();
        }

        @JavascriptInterface
        public void releaseTouchPoint(int id) {
            EventSender.this.releaseTouchPoint(id);
        }

        @JavascriptInterface
        public void touchEnd() {
            EventSender.this.touchEnd();
        }

        @JavascriptInterface
        public void touchCancel() {
            EventSender.this.touchCancel();
        }

        @JavascriptInterface
        public void clearTouchPoints() {
            EventSender.this.clearTouchPoints();
        }

        @JavascriptInterface
        public void cancelTouchPoint(int id) {
            EventSender.this.cancelTouchPoint(id);
        }
    };
}
