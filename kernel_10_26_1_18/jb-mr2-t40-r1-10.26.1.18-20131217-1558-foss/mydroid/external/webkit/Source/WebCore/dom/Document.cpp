...
#endif
#if ENABLE(TOUCH_EVENTS)
    , m_touchSensitiveRegionsDirty(false)
#endif
...
void Document::recalcStyle(StyleChange change)
{
    ...
    if (change == Force) {
        ...
        RefPtr<RenderStyle> documentStyle = CSSStyleSelector::styleForDocument(this, m_styleSelector ? m_styleSelector->fontSelector() : 0);        StyleChange ch = diff(documentStyle.get(), renderer()->style());
...
#if ENABLE(TOUCH_EVENTS)
    didRemoveAllTouchEventListeners();
#endif
}
...
bool Document::updateActiveStylesheets(StyleSelectorUpdateFlag updateFlag)
{
    ...
    if (requiresStyleSelectorReset)
        ...
        m_styleSelector->appendAuthorStylesheets(this, m_styleSheets->length(), newStylesheets);
        resetCSSFeatureFlags();
    }
...
void Document::didAddTouchEventListener(Node* node)
{
    markTouchSensitiveRegionsDirty();
    m_touchSensitiveNodes.add(node);
}

void Document::didRemoveTouchEventListener(Node* node)
{
    // We might have cleared all event listeners already.
    if (m_touchSensitiveNodes.remove(node))
        markTouchSensitiveRegionsDirty();
}

void Document::markTouchSensitiveRegionsDirty()
{
    if (m_touchSensitiveRegionsDirty)
        return;

    if (parentDocument())
        parentDocument()->markTouchSensitiveRegionsDirty();

    m_touchSensitiveRegionsDirty = true;
    if (Page* page = this->page())
        page->chrome()->client()->touchSensitiveRegionsChanged();
}

void Document::didRemoveAllTouchEventListeners()
{
    m_touchSensitiveRegionsDirty = false;
    m_touchSensitiveNodes.clear();

    if (m_touchSensitiveRegions.isEmpty())
        return;

    m_touchSensitiveRegions.clear();

    if (Page* page = this->page())
        page->chrome()->client()->touchSensitiveRegionsChanged();
}

static void collectRendererRects(RenderObject* renderer, const IntRect& containingRect, Vector<IntRect>& rendererRects)
{
    IntRect r = renderer->absoluteClippedOverflowRect();

    bool isNewRect = !r.isEmpty() && !containingRect.contains(r);
    if (isNewRect)
        rendererRects.append(r);

    for (RenderObject* child = renderer->firstChild(); child; child = child->nextSibling())
        collectRendererRects(child, isNewRect ? r : containingRect, rendererRects);
}

const Vector<IntRect>& Document::collectTouchSensitiveRegions()
{
    if (!m_touchSensitiveRegionsDirty)
        return m_touchSensitiveRegions;

    m_touchSensitiveRegions.clear();

    TouchSensitiveNodeMap::iterator i = m_touchSensitiveNodes.begin();
    TouchSensitiveNodeMap::iterator end = m_touchSensitiveNodes.end();
    for (; i != end; ++i) {
        ...
        collectRendererRects(renderer, IntRect(), m_touchSensitiveRegions);
    }
