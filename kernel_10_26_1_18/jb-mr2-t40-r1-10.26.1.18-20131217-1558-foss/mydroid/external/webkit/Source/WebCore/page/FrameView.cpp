...
bool FrameView::syncCompositingStateIncludingSubframes()
{
    ...
    for (HashSet<RefPtr<Widget> >::const_iterator current = viewChildren->begin(); current != end; ++current) {
        ...
        if (widget->isFrameView()) {
            bool synced = static_cast<FrameView*>(widget)->syncCompositingStateIncludingSubframes();
...
void FrameView::layout(bool allowSubtree)
{
    ...
    document->markTouchSensitiveRegionsDirty();
#endif
...
#if ENABLE(TOUCH_EVENTS)
void FrameView::collectTouchSensitiveRegions(Vector<IntRect>& touchRegions)
{
    touchRegions = frame()->document()->collectTouchSensitiveRegions();

    // Traverse visible child frames and collect the touch sensitive areas of their documents.
    Frame* currentFrame = frame();
iterateChildFrames:
    while ((currentFrame = currentFrame->tree()->traverseNext())) {
        if (!currentFrame->document())
            continue;

        if (!currentFrame->ownerRenderer() || currentFrame->ownerRenderer()->style()->visibility() != VISIBLE)
            continue;

        const Vector<IntRect> currentFrameRegions = currentFrame->document()->collectTouchSensitiveRegions();

        for (size_t i = 0; i < currentFrameRegions.size(); ++i) {
            IntRect rect = currentFrameRegions[i];
            // Adjust rect by the frame scroll positions of all the parent frames.
            for (Frame* f = currentFrame; f != frame(); f = f->tree()->parent()) {
                if (!f->view())
                    goto iterateChildFrames;

                rect.intersect(f->view()->boundsRect());
                if (rect.isEmpty())
                    break;

                rect = f->view()->convertToContainingView(rect);
            }

            if (!rect.isEmpty()) {
                // Un-adjust the scroll position of this frame, since touch regions are relative to
                // this frame top.
                rect.move(scrollX(), scrollY());
                touchRegions.append(rect);
            }
        }
    }
}
