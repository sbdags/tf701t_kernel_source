...
static const int PsuedoMainFrameDownladResourceSize = 60;
static const int PsuedoSubFrameDownladResourceSize = 25;
...
static bool isMainFrame(const Frame* frame) {
   return (!frame->tree() || !frame->tree()->parent());
}        
...
#endif
{
    if (frame() && isMainFrame(frame())) {
        m_usePsuedoRequestCount = true;
        m_psuedoRequestCount = PsuedoMainFrameDownladResourceSize;
    } else {
        m_usePsuedoRequestCount = false;
        m_psuedoRequestCount = PsuedoSubFrameDownladResourceSize;
    }
}
...
void CachedResourceLoader::incrementRequestCount(const CachedResource* res)
{
    ...
    // cancal psuedo
    m_usePsuedoRequestCount = true;    
    if (m_usePsuedoRequestCount && m_requestCount >= m_psuedoRequestCount)
        cancelPsuedoRequestCount();
}
...
void CachedResourceLoader::decrementRequestCount(const CachedResource* res)
{
    ...
    if (m_usePsuedoRequestCount && m_requestCount == 0)
        cancelPsuedoRequestCount();
    
    if (m_usePsuedoRequestCount && --m_psuedoRequestCount == 0)
        cancelPsuedoRequestCount();
}

int CachedResourceLoader::requestCount(bool psuedo)
{
    if (psuedo && m_usePsuedoRequestCount)
        return m_psuedoRequestCount;
...
void CachedResourceLoader::cancelPsuedoRequestCount()
{
    m_usePsuedoRequestCount = false;
    m_psuedoRequestCount = 0;
}
