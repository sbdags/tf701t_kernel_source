...
void CSSStyleSelector::appendAuthorStylesheets(Document* document, unsigned firstNew, const Vector<RefPtr<StyleSheet> >& stylesheets)
{
...
bool CSSStyleSelector::determineStylesheetSelectorScopes(CSSStyleSheet* stylesheet, HashSet<AtomicStringImpl*>& idScopes, HashSet<AtomicStringImpl*>& classScopes)
{
    ...
    for (size_t i = 0; i < size; i++) {
        StyleBase* rule = stylesheet->item(i);
        if (rule->isStyleRule()) {
...
bool CSSStyleSelector::SelectorChecker::determineSelectorScopes(const CSSSelectorList& selectorList, HashSet<AtomicStringImpl*>& idScopes, HashSet<AtomicStringImpl*>& classScopes)
{
