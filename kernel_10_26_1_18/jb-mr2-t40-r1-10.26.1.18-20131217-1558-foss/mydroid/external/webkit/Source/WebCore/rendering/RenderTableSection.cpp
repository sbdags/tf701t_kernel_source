...
{
    unsigned  nRows = m_gridRows;
    if (numRows > nRows) {
        if (numRows > static_cast<unsigned>(m_grid.size())) {
        ...
        for (unsigned r = nRows; r < numRows; r++) {
...
void RenderTableSection::setCellLogicalWidths()
{
    ...
    for (unsigned i = 0; i < m_gridRows; i++) {
...
int RenderTableSection::calcRowLogicalHeight()
{
    ...
    if (table()->isSingleColumn()) {
        ...
        for (unsigned r = 0; r < m_gridRows; r++)
    ...
    for (unsigned r = 0; r < m_gridRows; r++) {
        ...
        for (unsigned c = 0; c < totalCols; c++) {
            ...
            int indx = max(r - cell->rowSpan() + 1, 0u);
...
void RenderTableSection::appendColumn(unsigned pos)
{
    ...
    for (unsigned row = 0; row < m_gridRows; ++row)
