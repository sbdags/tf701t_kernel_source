...
void RenderTable::adjustLogicalHeightForCaption(RenderBlock* caption)
{
    ...
    caption->setLogicalLocation(caption->marginStart(), logicalHeight());
...
{
    for (unsigned i = 0; i < m_captions.size(); i++) {
        int captionLogicalHeight = m_captions[i]->logicalHeight() + m_captions[i]->marginBefore() + m_captions[i]->marginAfter();
