...
Length RenderTableCell::logicalWidthFromColumns(RenderTableCol* firstColForThisCell, Length widthFromStyle) const
{
    ...
    if (colWidthSum > 0)
        return Length(max(0, colWidthSum - borderAndPaddingLogicalWidth()), Fixed);
...
CollapsedBorderValue RenderTableCell::collapsedStartBorder() const
{
    ...
    if (colElt && startColEdge) {
        ...
        if (RenderTableCol* enclosingColumnGroup = colElt->enclosingColumnGroupIfAdjacentBefore()) {
            result = chooseBorder(result, CollapsedBorderValue(&enclosingColumnGroup->style()->borderStart(), enclosingColumnGroup->style()->visitedDependentColor(start), BCOLGROUP));
            if (!result.exists())
                return result;
        }
    }
...
CollapsedBorderValue RenderTableCell::collapsedEndBorder() const
{
    ...
    if (colElt && endColEdge) {
        ...
        if (RenderTableCol* enclosingColumnGroup = colElt->enclosingColumnGroupIfAdjacentAfter()) {
            result = chooseBorder(result, CollapsedBorderValue(&enclosingColumnGroup->style()->borderEnd(), enclosingColumnGroup->style()->visitedDependentColor(end), BCOLGROUP));
            if (!result.exists())
                return result;
        }
    }
    ...
    if (!currSection) {
        ...
        if (colElt) {
            ...
            if (RenderTableCol* enclosingColumnGroup = colElt->enclosingColumnGroup()) {
                result = chooseBorder(result, CollapsedBorderValue(&enclosingColumnGroup->style()->borderBefore(), enclosingColumnGroup->style()->visitedDependentColor(before), BCOLGROUP));
                if (!result.exists())
                    return result;
            }
        }
...
CollapsedBorderValue RenderTableCell::collapsedAfterBorder() const
{
    ...
    if (!currSection) {
        ...
        if (colElt) {
            ...
            if (RenderTableCol* enclosingColumnGroup = colElt->enclosingColumnGroup()) {
                result = chooseBorder(result, CollapsedBorderValue(&enclosingColumnGroup->style()->borderAfter(), enclosingColumnGroup->style()->visitedDependentColor(after), BCOLGROUP));
                if (!result.exists())
                    return result;
            }
        }
