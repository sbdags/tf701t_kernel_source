...
RenderLayer* RenderLayer::clipRectRootLayer() const
{
    for (const RenderLayer* current = this; current; current = current->parent()) {
...
void RenderLayer::scrollTo(int x, int y)
{
    ...
    if (view) {
        ...
        renderer()->document()->markTouchSensitiveRegionsDirty();
#endif
                        ...
                        PaintLayerFlags paintFlags)
{
    ...
    if (paintsWithTransform(paintBehavior) && !(paintFlags & PaintLayerAppliedTransform)) {
        ...
        if (parent()) {
            clipRect = backgroundClipRect(rootLayer, (paintFlags & PaintLayerTemporaryClipRects) ? TemporaryClipRects : PaintingClipRects);
    ...
    calculateRects(rootLayer, (paintFlags & PaintLayerTemporaryClipRects) ? TemporaryClipRects : PaintingClipRects, paintDirtyRect, layerBounds, damageRect, clipRectToApply, outlineRect);
    int x = layerBounds.x();
    int y = layerBounds.y();
    int tx = x - renderBoxX();
    int ty = y - renderBoxY();
...
bool RenderLayer::hitTest(const HitTestRequest& request, HitTestResult& result)
{
    ...
    if (!request.ignoreClipping() && frameVisibleRect(renderer()).contains(result.point()))
...
bool RenderLayer::hitTestInstant(const HitTestRequest& request, HitTestResult& result)
{
    renderer()->document()->updateLayout();
                                       ...
                                       const HitTestingTransformState* transformState, double* zOffset)
{
    ...
    if (transform() && !appliedTransform) {
        // Make sure the parent's clip rects have been calculated.
        if (parent()) {
            IntRect clipRect = backgroundClipRect(rootLayer, useTemporaryClipRects ? TemporaryClipRects : RootRelativeClipRects, IncludeOverlayScrollbarSize);
    ...
    IntRect fgRect;
    IntRect outlineRect;
    calculateRects(rootLayer, useTemporaryClipRects ? TemporaryClipRects : RootRelativeClipRects, hitTestRect, layerBounds, bgRect, fgRect, outlineRect, IncludeOverlayScrollbarSize);
...
void RenderLayer::updateClipRects(const RenderLayer* rootLayer, ClipRectsType clipRectsType, OverlayScrollbarSizeRelevancy relevancy)
{
    ...
    if (parentLayer)
        parentLayer->updateClipRects(rootLayer, clipRectsType, relevancy);

    ClipRects clipRects;
    calculateClipRects(rootLayer, clipRectsType, clipRects, relevancy);
...
void RenderLayer::calculateClipRects(const RenderLayer* rootLayer, ClipRectsType clipRectsType, ClipRects& clipRects, OverlayScrollbarSizeRelevancy relevancy) const
{
    if (!parent()) {
    ...
    if (parentLayer) {
        if (useCached && parentLayer->clipRects(clipRectsType))
            ...
            parentLayer->calculateClipRects(rootLayer, clipRectsType, clipRects);
    }
    else
        clipRects.reset(PaintInfo::infiniteRect());
...
void RenderLayer::parentClipRects(const RenderLayer* rootLayer, ClipRectsType clipRectsType, ClipRects& clipRects, OverlayScrollbarSizeRelevancy relevancy) const
{
    ASSERT(parent());
    if (clipRectsType == TemporaryClipRects) {
        parent()->calculateClipRects(rootLayer, clipRectsType, clipRects, relevancy);
        return;
    }

    parent()->updateClipRects(rootLayer, clipRectsType, relevancy);
    clipRects = *parent()->clipRects(clipRectsType);
}

IntRect RenderLayer::backgroundClipRect(const RenderLayer* rootLayer, ClipRectsType clipRectsType, OverlayScrollbarSizeRelevancy relevancy) const
{
    ASSERT(parent());

    ClipRects parentRects;
    // We cannot cache rects if the layer has a composited parent but at the same time is painted to
    // a different composited layer.
    if (clipRectsType != TemporaryClipRects && clipRectRootLayer() != rootLayer)
        parentClipRects(rootLayer, TemporaryClipRects, parentRects, relevancy);
    else
        parentClipRects(rootLayer, clipRectsType, parentRects, relevancy);

    IntRect backgroundRect = renderer()->style()->position() == FixedPosition ? parentRects.fixedClipRect() :
...
void RenderLayer::calculateRects(const RenderLayer* rootLayer, ClipRectsType clipRectsType, const IntRect& paintDirtyRect, IntRect& layerBounds,
                                 IntRect& backgroundRect, IntRect& foregroundRect, IntRect& outlineRect, OverlayScrollbarSizeRelevancy relevancy) const
{
    if (rootLayer != this && parent()) {
        backgroundRect = backgroundClipRect(rootLayer, clipRectsType, relevancy);
...
IntRect RenderLayer::childrenClipRect() const
{
    ...
    calculateRects(clippingRootLayer, PaintingClipRects, renderView->documentRect(), layerBounds, backgroundRect, foregroundRect, outlineRect);
...
IntRect RenderLayer::calculateLayerBounds(const RenderLayer* layer, const RenderLayer* ancestorLayer)
{
    ...
    if (layer->renderer()->isRoot()) {
        ...
        if (FrameView* frameView = layer->renderer()->view()->frameView()) {
            int contentsWidth = frameView->contentsWidth();
            int contentsHeight = frameView->contentsHeight();
