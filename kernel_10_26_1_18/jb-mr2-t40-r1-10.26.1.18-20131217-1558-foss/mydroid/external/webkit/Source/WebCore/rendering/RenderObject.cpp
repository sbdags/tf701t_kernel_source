...
void RenderObject::repaintUsingContainer(RenderBoxModelObject* repaintContainer, const IntRect& r, bool immediate)
{
    ...
    if (repaintContainer->isRenderView()) {
        ...
        if (!v->hasLayer() || !v->layer()->isComposited() || v->layer()->backing()->paintsIntoWindow()) {
...
void RenderObject::insertedIntoTree()
{
    ...
    if (!isFloatingOrPositioned() && parent()->childrenInline())
...
bool RenderObject::isUsingInstantDic = false;
void RenderObject::setIsUsingInstantDic(bool b)
{
    isUsingInstantDic = b;
}
