...
public:
    ...
    bool isFixed() const { return isFixed(renderer()->style()); }
    bool isFixed(const RenderStyle* style) const { return renderer()->isPositioned() && style->position() == FixedPosition; }

    // If fixed elements are composited, they will be containing children
    bool isStackingContext(const RenderStyle* style)  const {
#if ENABLE(ANDROID_OVERFLOW_SCROLL)
      if (hasOverflowScroll())
          return true;
#endif
      return !style->hasAutoZIndex() || renderer()->isRenderView() || (isComposited() && isFixed(style));
    }
#else
#if ENABLE(ANDROID_OVERFLOW_SCROLL)
    bool isStackingContext(const RenderStyle* style) const { return !style->hasAutoZIndex() || renderer()->isRenderView() || hasOverflowScroll(); }
#else
    ...
    RenderLayer* clipRectRootLayer() const;
    ...
    bool hitTestInstant(const HitTestRequest&, HitTestResult&);
    ...
    void calculateRects(const RenderLayer* rootLayer, ClipRectsType, const IntRect& paintDirtyRect, IntRect& layerBounds,
    ...
    void updateClipRects(const RenderLayer* rootLayer, ClipRectsType, OverlayScrollbarSizeRelevancy = IgnoreOverlayScrollbarSize);
    // Compute and return the clip rects. If useCached is true, will used previously computed clip rects on ancestors
    // (rather than computing them all from scratch up the parent chain).
    void calculateClipRects(const RenderLayer* rootLayer, ClipRectsType, ClipRects&, OverlayScrollbarSizeRelevancy = IgnoreOverlayScrollbarSize) const;
    ...
    static IntRect calculateLayerBounds(const RenderLayer*, const RenderLayer* ancestorLayer);
...
private:
    ...
    void parentClipRects(const RenderLayer* rootLayer, ClipRectsType, ClipRects&, OverlayScrollbarSizeRelevancy = IgnoreOverlayScrollbarSize) const;
    IntRect backgroundClipRect(const RenderLayer* rootLayer, ClipRectsType, OverlayScrollbarSizeRelevancy = IgnoreOverlayScrollbarSize) const;
