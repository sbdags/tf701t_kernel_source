...
HTMLPlugInElement::HTMLPlugInElement(const QualifiedName& tagName, Document* doc)
    ...
    , m_isListeningTouchEvents(false)
#endif
{
}
...
#if ENABLE(TOUCH_EVENTS)
void HTMLPlugInElement::attach()
{
    HTMLFrameOwnerElement::attach();

    if (m_isListeningTouchEvents) {
...
#if ENABLE(TOUCH_EVENTS)
    if (attached() && m_isListeningTouchEvents)
...
#if ENABLE(TOUCH_EVENTS)
void HTMLPlugInElement::setIsListeningTouchEvents(bool listening)
{
    if (m_isListeningTouchEvents == listening)
        return;

    m_isListeningTouchEvents = listening;
    if (attached()) {
        if (m_isListeningTouchEvents) {
            document()->addListenerTypeIfNeeded(eventNames().touchstartEvent);
            document()->didAddTouchEventListener(this);
