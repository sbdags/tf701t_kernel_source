...
static WebViewCore* webViewCoreFromClient(GraphicsLayerClient* client)
{
    if (RenderLayer* layer = renderLayerFromClient(client))
        return WebViewCore::getWebViewCore(layer->renderer()->frame()->view());
    return 0;
}
...
GraphicsLayerAndroid::GraphicsLayerAndroid(GraphicsLayerClient* client) :
    ...
    m_needsSyncMediaLayerContents(false),
    m_newImage(false),
    m_image(0),
...
GraphicsLayerAndroid::~GraphicsLayerAndroid()
{
    ...
    if (m_mediaLayer)
        m_mediaLayer->didDetachFromView();
...
void GraphicsLayerAndroid::updatePositionedLayers()
{
    ...
    if (view->isRenderIFrame() && !m_contentLayer->isIFrame()) {
        ASSERT(!m_contentLayer->isFixedBackground());
...
void GraphicsLayerAndroid::setDrawsContent(bool drawsContent)
{
    ...
    if (m_drawsContent)
        ...
        if (m_foregroundLayer)
            m_foregroundLayer->setContent(0);
        m_contentLayerContent.reset();
        m_foregroundLayerContent.reset();
        m_needsRepaint = false;
    }
    askForSync();
}
...
void GraphicsLayerAndroid::updateMediaLayer()
{
    m_mediaLayer->setPosition(m_contentsRect.x(), m_contentsRect.y());
    m_mediaLayer->setSize(m_contentsRect.width(), m_contentsRect.height());
}
...
void GraphicsLayerAndroid::updateFixedBackgroundLayers() {
    ...
    if (!view->style()->hasFixedBackgroundImage())
        return;
    // Can't use the fixed background layer for iframes since they have their own layer class.
...
{
    ALOGV("(%x) repaint(), gPaused(%d) m_needsRepaint(%d) ",
          this, gPaused, m_needsRepaint);
    ...
    if (!gPaused && !m_image) {
        ...
        if (m_foregroundLayer && !m_contentLayer->isFixedBackground()) {
            ...
            PictureLayerContent* layerContent = PictureLayerContent::createAndBeginRecording(layerBounds.size());
            if (layerContent) {
                  PaintingPhase phase(this);
                  PlatformGraphicsContextSkia platformContext(layerContent->recordingCanvas());
...
void GraphicsLayerAndroid::setContentsNeedsDisplay()
{
    ...
    if (m_mediaLayer->handleNeedsDisplay())
        return;

    askForMediaLayerContentsSync();
}

void GraphicsLayerAndroid::setContentsRect(const IntRect& rect)
{
...
void GraphicsLayerAndroid::removeAnimation(const String& name)
{
    m_contentLayer->removeAnimation(name);
    askForSync();
}
...
void GraphicsLayerAndroid::setContentsToMedia(PlatformLayer* mediaLayer)
{
    ...
    SkSafeUnref(m_mediaLayer);
    m_mediaLayer = mediaLayer;
    SkSafeRef(m_mediaLayer);

    if (m_mediaLayer)
        m_mediaLayer->didAttachToView(webViewCoreFromClient(m_client));

    m_needsSyncChildren = true;
    askForSync();

    // The canvas layer might not be synched when the element is attached to the layer tree.
    // This is why we need setContentsNeedsDisplay().
    if (m_mediaLayer) {
        updateMediaLayer();
        setNeedsDisplay();
...
{
    setContentsToMedia(canvasLayer);
}
...
void GraphicsLayerAndroid::askForMediaLayerContentsSync()
{
    if (m_needsSyncMediaLayerContents)
        return;

    m_needsSyncMediaLayerContents = true;
...
void GraphicsLayerAndroid::syncMediaLayerContents()
{
    if (!m_needsSyncMediaLayerContents)
        return;
...
void GraphicsLayerAndroid::notifyClientAnimationStarted(double startTime)
{
    for (unsigned int i = 0; i < m_children.size(); i++)
        static_cast<GraphicsLayerAndroid*>(m_children[i])->notifyClientAnimationStarted(startTime);
