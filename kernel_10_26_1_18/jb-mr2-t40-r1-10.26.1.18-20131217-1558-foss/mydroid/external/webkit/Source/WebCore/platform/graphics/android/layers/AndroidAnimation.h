...
private:
    double checkIterationsAndProgress(double time);
    bool hasStarted(double time) const
    {
        ASSERT(m_beginTime > std::numeric_limits<double>::epsilon());
        return time > m_beginTime;
    }

    bool hasEnded(double time) const
    {
        return m_iterationCount != WebCore::Animation::IterationCountInfinite
            && time > (m_beginTime + (m_iterationCount * m_duration));
    }
};
