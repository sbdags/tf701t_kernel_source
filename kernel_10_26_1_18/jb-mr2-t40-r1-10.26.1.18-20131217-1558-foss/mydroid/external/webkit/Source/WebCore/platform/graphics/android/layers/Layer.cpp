...
void Layer::draw(SkCanvas* canvas, android::DrawExtra* extra, SkScalar opacity, bool showVisualIndicator)
{
#if 0
    SkString str1, str2;
...
//        SkDebugf("---- abort drawing %p opacity %g\n", this, opacity);
    ...
    onDraw(canvas, opacity, extra, (showVisualIndicator ? PaintVisualIndicator : 0) | FlattenedLayers);
    ...
    if (count > 0) {
        ...
        for (int i = 0; i < count; i++) {
            getChild(i)->draw(canvas, extra, opacity, showVisualIndicator);
        }
    }
}
