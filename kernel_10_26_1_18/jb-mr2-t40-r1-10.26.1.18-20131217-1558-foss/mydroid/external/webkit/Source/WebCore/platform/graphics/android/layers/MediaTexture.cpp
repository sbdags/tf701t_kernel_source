...
{
    {
        ...
        if (!m_contentTexture) {
            m_contentTexture = createTexture(true);
          ...
          const SkRect& mediaBounds)
{
    ...
    android::sp<android::GraphicBuffer> buf = m_contentTexture->surfaceTexture->getCurrentBuffer();

    TextureQuadData::ContentFlags contentFlags = TextureQuadData::ContentFlagsNone;
    android::PixelFormat f = buf->getPixelFormat();
    if (android::PIXEL_FORMAT_RGBX_8888 == f
        || android::PIXEL_FORMAT_RGB_888 == f
        || android::PIXEL_FORMAT_RGB_565 == f)
                                 ...
                                 const SkRect& dimensions)
{
    ...
    if (m_contentTexture->nativeWindow.get() == window)
        m_contentTexture->surfaceTexture->setDefaultBufferSize((int)dimensions.width(), (int)dimensions.height());
}
...
MediaTexture::TextureWrapper* MediaTexture::createTexture(bool forceTextureAsync)
{
    TextureWrapper* wrapper = new TextureWrapper();

    // populate the wrapper
    glGenTextures(1, &wrapper->textureId);
    wrapper->surfaceTexture = new android::GLConsumer(wrapper->textureId, !forceTextureAsync);
    wrapper->nativeWindow = new android::Surface(wrapper->surfaceTexture->getBufferQueue());
